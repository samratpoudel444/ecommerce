...
async function signInUsers(req, resp, next) {
  ...
  try {
    ...
    if (data.length === 0) {
      return resp.status(400).json({ error: "The provided email is not registered." });
    }

    // Compare provided password with the stored hash
    const match = await bcrypt.compare(password, data[0].password);
    if (!match) {
      return resp.status(400).json({ error: "The provided password is incorrect." });
    }
    ...
    const token = jwt.sign(jwtData, process.env.SECRET_KEY, { expiresIn: "1h" });

    // Return the JWT token
    return resp.status(200).json({ message: "Login successful", token });
  } catch (error) {
    console.error(error);
    return resp.status(500).json({ error: "Server error occurred during login." });
  }
}
